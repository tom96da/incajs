...
A Vue port of [`crates/inca-bridge/examples/click_counter.rs`](../../crates/inca-bridge/examples/click_counter.rs):
...
INCA_HOST_BIN="$(pwd)/target/debug/inca-host" pnpm --filter click_counter dev
```
...
```sh
cargo run -p inca-host -- examples/click_counter/dist/bundle.js
```
